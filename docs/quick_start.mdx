...
Distance-based shapes in **Shape-Z**—like `Cylinder`, `RectFrustum`, `NgonFrustum`, `Cone`, `Capsule`, and `Vesica`—are defined between two points (`from`, `to`). Others like `Sphere` and `Box` use a center point (`at`) and a size.

Inside a `distance` block, you work with normalized shape-local coordinates:
- `d`: along the shape's axis (from 0 to 1)
- `u`, `v`: horizontal and vertical coordinates within the cross-section at each `d`

These coordinates make it easy to apply consistent details, deformations, and materials regardless of size or orientation.

A `volume { ... }` block defines the content inside the shape. You can also optionally define:
- `profile_offset { ... }` to deform the surface
- `profile_scale { ... }` to vary thickness over the shape

This enables complex modeling like bulging bottles, tapering beams, or ornamental pillars with just a few lines of code.

<ShpzPlayground
    src = "/examples/quick5.shpz"
    height={500}
    caption="Distance Shapes with Profiles"
/>
